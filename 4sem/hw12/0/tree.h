...
{
	private:
	    TreeNode <T, T1> *Root = nullptr;
	    TreeNode <T, T1> *Current = nullptr;
	...
	public:
	    ...
	    //
	    void delete_tree(TreeNode <T, T1> *tmp)
	    {
		if(tmp != nullptr)
		{
		...
		{
		    TreeNode <T, T1>* tmp = Current->Brother;
		...
		TreeNode <T, T1> *tmp = nullptr;
		tmp = new TreeNode <T, T1>;
		if (tmp == nullptr)
		{
	    ...
	    void print(TreeNode <T, T1> *root, int max_print, FILE* out)
	    {
		if(root == nullptr)
		{
		    printf("    Empty tree!\n");
	    ...
	    void add(TreeNode <T, T1> *tmp, TreeNode <T, T1> *root)
	    {
		/*prinf ("tmp = %s %d\n", tmp->Front->name.get(), tmp->Front->value);
		prinf ("root = %s %d\n", root->Front->name.get(), root->Front->value);
		printf(" bool = tmp < root = %d\n", (*tmp < *root));*/
		if (root->Child == nullptr)
		{
		    root->Child = tmp;
		}
	    ...
	    void addRoot()
	    {
		TreeNode <T, T1>* tmp = new TreeNode <T, T1>;
		if(Root == nullptr)
		{
		    Root = Current = tmp;
		}
	    ...
	    TreeNode <T, T1>* del_sub_level()
	    {
		delete_tree(Current -> Brother );
		return Root;
	    }

	    TreeNode <T, T1>* del_sub_tree()
	    {
		delete_tree(Current -> Child);
		return Root;
	    }
