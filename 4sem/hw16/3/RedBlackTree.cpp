...
bool RedBlackTree::isContain(ListNode * reference, bool selector)
{
    RedBlackTreeNode *tmp = Root;
    while (tmp != NILL)
    {
	if(*tmp == *reference)
	{
	    return (selector ? selector : tmp->NameSearch->isContain(reference));
	}
...
RedBlackTreeNode* RedBlackTree::binaryInsertion(RedBlackTreeNode* root, RedBlackTreeNode *tmp)
{
    ...
    if (*tmp < *(root->NameSearch->getRoot()->Data))
    {
        root->LeftChild  = binaryInsertion(root->LeftChild, tmp);
	root->LeftChild->Parent = root;
    }
    else if (*tmp > *root->NameSearch->getRoot()->Data)
    {
...
void RedBlackTree::deleteValue(ListNode * reference)
{
	...
	/*if(found->nameSearch->getRoot()->LeftChild == found->nameSearch->NILL || found->nameSearch->getRoot()->LeftChild == found->nameSearch->NILL)
	{
	    RedBlackTreeNode * buff = binaryDeletion(found);
	    delete buff;
	}*/
    }
}
