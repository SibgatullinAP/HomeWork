...
/*ListNode * RedBlackTree::search(Command & command, int flagType, int flag, ListNode *& begin)
{
    (void)flag;
    ...
    while (tmp != NILL)
    {
	if(tmp->Group == command.getGroup())
	{
	    break;
	}
	else if(tmp->Group < command.getGroup())
	{
	    tmp = tmp->RightChild;
	}
	else
    ...
    begin = tmp->PhoneSearch->search(command, flagType, flag, begin);
...
void RedBlackTree::traverse(Command &command, RedBlackTreeNode *tmp, ListNode *& begin, int flagType)
{
    if (tmp == NILL)
    {
	return;
