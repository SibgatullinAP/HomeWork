...
class database
{
	...
	void docommand(command &x, int qwer)
	{
		set_fd(qwer);
		switch (x.type)
		{
	...
	void select(command &x)
	{
		...
		case ARG_NONE:
			...
			while (rr)
			{
                     switch(x.take1)
                     {
                         case TAKE_ALL:   PrinttoClient(filedescr,"%s %d %d",rr->get_name(),rr->phone,rr->group); break;
                         case TAKE_GROUP: PrinttoClient(filedescr,"%d ",rr->group); break;
                         case TAKE_PHONE: PrinttoClient(filedescr,"%d ",rr->phone); break;
                         case TAKE_NAME:  PrinttoClient(filedescr,"%s ",rr->get_name()); break;
                         case TAKE_NONE: break;
                     }
                     switch(x.take2)
                     {
                        case TAKE_ALL: break;
                         case TAKE_GROUP: PrinttoClient(filedescr,"%d \n",rr->group); break;
                         case TAKE_PHONE: PrinttoClient(filedescr,"%d \n",rr->phone); break;
                         case TAKE_NAME:PrinttoClient(filedescr,"%s \n",rr->get_name()); break;
                         case TAKE_NONE:PrinttoClient(filedescr,"\n"); break;
                     }
                             rr=rr->next;
                             }
                             return;
		 ...
		 void printstack(command &x)
		 {
			 if(stack)
			 {
				 ...
				 while(k)
				 {
					 ...
					 switch(x.take1)
					{
					case TAKE_ALL:   PrinttoClient(filedescr,"%s %d %d",u->get_name(),u->phone,u->group);  break;
					case TAKE_GROUP: PrinttoClient(filedescr,"%d ",u->group); break;
					case TAKE_PHONE: PrinttoClient(filedescr,"%d ",u->phone); break;
					case TAKE_NAME:  PrinttoClient(filedescr,"%s ",u->get_name()); break;
					case TAKE_NONE: break;
					}
					switch(x.take2)
					{
