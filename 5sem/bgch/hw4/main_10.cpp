...
int main (int argc, char *argv[])
{
  ...
  if (argc != 4 && argc != 5 && argc != 6)
    {
      printf ("[ERROR] Please, use: %s M-th_element Matrix_size Tau (File_name_matrix) (File_name_X_0)\n", argv[0]);
      return -1;
    }
  ...
  double tau = 0;
  if (sscanf (argv[3], "%lf", &tau) != 1)
    {
      printf ("[ERROR] Tau should be a real number!\n");
      return -1;
    }
  ...
  if (X_0 == nullptr)
    {
      delete [] matrix_A;
      delete [] X;
      printf ("[ERROR] Not enough memmory for Vector X_0!\n");
      return -2;
    }
  ...
  if (R == nullptr)
    {
      ...
      delete [] X;
      delete [] X_0;
      printf ("[ERROR] Not enough memmory for Vector R!\n");
      return -2;
    }
  ...
  bzero (R, sizeof (double) * matrix_size);
  bzero (X, sizeof (double) * matrix_size);

  init_B (matrix_A, B, matrix_size);
  ...
  print (B, matrix_size, 1, MAX_PRINT);

  double time = clock ();
  solve_10 (matrix_A, X_0, X, B, matrix_size, m_th, R, tau);
  time = (clock () - time) / CLOCKS_PER_SEC;

  printf("[OUTPUT] Vector X_m:\n");
  print (X, matrix_size, 1, MAX_PRINT);
  printf ("[OUTPUT] Residual: %10.3e\n", discrepancy_rate_norm_1(matrix_A, X, B, matrix_size));
  printf ("[OUTPUT] Error_rate: %10.3e\n", error_rate_norm_1(X, matrix_size));
