  ...
  for (int i = 0; i < m_th; i++)
    {
      ...
      if (gauss_elimination_LD(matrix_A, R, X, matrix_size) < 0)
        {
          printf ("D is singular matrix!\n");
          return;
        }
  ...
  for (int i = 0; i < m_th; i++)
    {
      ...
      if (gauss_elimination_RD (matrix_A, R, X, matrix_size) < 0)
        {
          printf ("D is singular matrix!\n");
          return;
        }
  ...
  for (int i = 0; i < m_th; i++)
    {
      ...
      double *D = matrix_A;
      for (int j = 0; j < matrix_size; j++)
        X[j] *= D[j + matrix_size * j];

      if (gauss_elimination_RD (matrix_A, X, R, matrix_size) < 0)
        {
          printf ("D is singular matrix!\n");
          return;
        }
...
{
  double *D = matrix;
  for (int i = 0; i < matrix_size; i++, D += matrix_size)
    {
      double sum = 0;
      for (int j = 0; j < i; j++)
        sum += answer[j] * D[j];
...
{
  double *D = matrix + matrix_size * matrix_size - matrix_size;
  for (int i = matrix_size - 1; i >= 0; i--, D -= matrix_size)
    {
      double sum = 0;
