...
int merge_arrays(double *, int, double *, int, double*);
...
int main(int argc, char *argv[]){
    ...
    t=clock();
    s=merge_arrays(a,n, b, m, c);
    t=clock()-t;
    printf("Size of new array c: %i\n", s);
    printf("New array c:\n");   print_array(c, s);
	printf("Elapsed=%.2f\n", t/CLOCKS_PER_SEC);
	free(a); free(b); free(c);
	return 0;
...
int merge_arrays(double *a, int n, double *b, int m, double *c){
    int i=0, j=0, s=0;
    while(i<n || j<m){
        if(i<n && j<m && a[i]<b[j]) {c[s]=a[i]; i++; s++;}
        if(i<n && j<m && a[i]>b[j]) {c[s]=b[j]; j++; s++;}
        if(i<n && j<m && !(a[i]<b[j] || a[i]>b[j])) { c[s]=a[i]; s++; i++; j++; }
	if(i<n && j>=m) { c[s]=a[i]; i++; s++;}
	if(i>=n && j<m) { c[s]=b[j]; j++; s++;}  
    }
    return s;
}
