...
struct PDE_gas_info {
  PDE_gas_info (double mu) : m_mu (mu) {};
...
public:
  ...
  double du2_dx (double t, double x) const {return 8 * std::numbers::pi * std::pow(std::cos(2 * std::numbers::pi * t), 2) * std::cos(4 * std::numbers::pi * x) * std::sin(4 * std::numbers::pi * x);}
  ...
  double p (double r, double c = 1, double gamma = 1) const {return c * std::pow(r, gamma);}
  double dp_dr(double r, double c = 1, double gamma = 1) const {return c * gamma * std::pow(r, gamma - 1);}
};
