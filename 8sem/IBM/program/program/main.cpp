...
int main(int argc, const char *argv[]) {
  ...
  tex_table V1_table (col_limit + 1, row_limit + 1, string_format("Table of norms for V1. $\\mu = %1.4lf$ \\, $C = %1.4lf$, $\\gamma = %1.4lf$", problem.m_mu, problem.m_C, problem.m_gamma));
  tex_table V2_table (col_limit + 1, row_limit + 1, string_format("Table of norms for V2. $\\mu = %1.4lf$ \\, $C = %1.4lf$, $\\gamma = %1.4lf$", problem.m_mu, problem.m_C, problem.m_gamma));
  std::string path_to_tables;

  init_table_metadata(table_time);
  init_table_metadata(H_table);
  init_table_metadata(V1_table);
  init_table_metadata(V2_table);
          ...
          for (int col = 0; col < col_limit; col++)
            {
              ...
              math_vec ethalon_V1, ethalon_V2;
              ethalon_V1 = ethalon_V2 = solver.get_ethalon_V();

              math_vec answer_H = solver.get_answer_H();
              math_vec answer_V1, answer_V2;
              answer_V1 = answer_V2 = solver.get_answer_V();

              math_vec diff_H = vec_diffs(ethalon_H, answer_H);
              math_vec diff_V1 = vec_diffs(ethalon_V1, answer_V1);
              math_vec diff_V2 = vec_diffs(ethalon_V2, answer_V2);

              fill_table_value (H_table, row + 1, col + 1, norm_c(diff_H), norm_l2(diff_H, solver.m_grid.dx()), norm_w21(diff_H, solver.m_grid.dx()));
              fill_table_value (V1_table, row + 1, col + 1, norm_c(diff_V1), norm_l2(diff_V1, solver.m_grid.dx()), norm_w21(diff_V1, solver.m_grid.dx()));
              fill_table_value (V1_table, row + 1, col + 1, norm_c(diff_V2), norm_l2(diff_V2, solver.m_grid.dx()), norm_w21(diff_V2, solver.m_grid.dx()));
            }
        }

      path_to_tables = "/home/pam/HomeWork/8sem/Numerical_analysis/hw1/tables/";
    }

  std::string h_filename = string_format("H_norms_mu%1.3lf_C%1.3lf_gamma%1.3lf.tex", problem.m_mu, problem.m_C, problem.m_gamma);
  std::string v1_filename = string_format("V1_norms_mu%1.3lf_C%1.3lf_gamma%1.3lf.tex", problem.m_mu, problem.m_C, problem.m_gamma);
  std::string v2_filename = string_format("V2_norms_mu%1.3lf_C%1.3lf_gamma%1.3lf.tex", problem.m_mu, problem.m_C, problem.m_gamma);

  table_time.print_to_tex(path_to_tables + "Times.tex", "w");
  H_table.print_to_tex(path_to_tables + h_filename, "w");
  V1_table.print_to_tex(path_to_tables + v1_filename, "w");
  V2_table.print_to_tex(path_to_tables + v2_filename, "w");
