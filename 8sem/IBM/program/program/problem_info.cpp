...
bool CDS_solver::solve() {
  ...
  math_vec V_1_L (dim - 1);
  math_vec V_1_D (dim);
  math_vec V_1_R (dim - 1);
  math_vec V_1_rhs (dim);
  math_vec V_1_prev (dim);

  math_vec V_2_L (dim - 1);
  math_vec V_2_D (dim);
  math_vec V_2_R (dim - 1);
  math_vec V_2_rhs (dim);
  math_vec V_2_prev (dim);
  ...
  auto f_1 = [this](double t, double x) -> double {
    return (r(t, x) * du_dt(t, x) + r(t, x) * u(t, x) * du_dx(t, x) + dp_dr(r(t, x)) * dr_dx(t, x) - m_pde_info.m_mu * d2u_dxdx(t, x)) / r(t, x);
  };

  auto f_2 = [this](double t, double x) -> double {
    return (r(t, x) * du_dt(t, x) + r(t, x) * u(t, x) * du_dx(t, x) + dp_dr(r(t, x)) * dr_dx(t, x) - m_pde_info.m_mu * d2u_dxdx(t, x)) / r(t, x);
  };
  ...
  for (int timestep = 0; timestep < timesteps; timestep++) {
    ...
    for (int m = 1; m < dim - 1; m++) {
      ...
      V_1_L[m - 1] = -(V_prev[m] + V_prev[m - 1]) / (6. * h) - tilde_mu / (h * h);
      V_1_D[m] = 1. / tau + (2 * tilde_mu) / (h * h);
      V_1_R[m] = (V_prev[m] + V_prev[m + 1]) / (6. * h) - tilde_mu / (h * h);
    ...
    for (int m = 1; m < dim - 1; m++) {
      ...
      // fill internal part V
      V_2_L[m - 1] = -(V_prev[m] + V_prev[m - 1]) / (6. * h) - tilde_mu / (h * h);
      V_2_D[m] = 1. / tau + (2 * tilde_mu) / (h * h);
      V_2_R[m] = (V_prev[m] + V_prev[m + 1]) / (6. * h) - tilde_mu / (h * h);
    ...
    // Solving matrix
    H_prev = solve_matrix(H_L, H_D, H_R, H_rhs);
    V_prev = solve_matrix(V_L, V_D, V_R, V_rhs);
    }
