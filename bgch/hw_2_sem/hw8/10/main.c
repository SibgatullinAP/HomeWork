...
int main(int argc, char **argv){
	...
	if(!( u=(double*)malloc((n*sizeof(double)))) ) { free(a); free(x0); free(x); free(b); free(r); return printf("No enough memory for R\n"), 2; }

	if(nameA){	if( errors( read_matrix(a, n, n, nameA) , nameA) )	{	free(a); free(x0); free(x); free(b); free(r); free(u); return 3;	}}	else init_matrix(a, n, n, &fa);
	if(nameX0){	if( errors( read_matrix(x0,n,1,nameX0) , nameX0) )  {	free(a); free(x0); free(x); free(b); free(r); free(u); return 3;	}}	else init_matrix(x0,n,1, &fx0);
	if(nameB){	if( errors( read_matrix(b, n, 1, nameB) , nameB) )	{	free(a); free(x0); free(x); free(b); free(r); free(u); return 3;	}}	else init_matrix(b, n, 1, &fb);
	...
	m--;
	t=clock();
	solve(a, x0, x, b, r, u, m, n);
	t=clock()-t;
	printf("\nANSWER...\n");
	printf("\nVector  x:\n"); print_matrix( x, n, 1);
 	printf("\nTime: %lf\n", t/CLOCKS_PER_SEC);
	free(a); free(x0); free(x); free(b); free(r); free(u);
	return 0;
}
