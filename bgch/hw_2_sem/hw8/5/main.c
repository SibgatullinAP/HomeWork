...
int main(int argc, char **argv){
	...
	if(nameA){	if( errors( read_matrix(a, n, n, nameA) , nameA) )	{	free(a); free(x0); free(x); free(b); return 3;	}}	else init_matrix(a, n, n, &fa);
	for(i=0; i<n*n; i+=n+1) if(a[i]<=0 && a[i]>=0) {	free(a); free(x0); free(x); free(b); printf("0 in diagonal A"); return 3;	}
	if(nameX0){	if( errors( read_matrix(x0,n,1,nameX0) , nameX0) )  {	free(a); free(x0); free(x); free(b); return 3;	}}	else init_matrix(x0,n,1, &fx0);
	if(nameB){	if( errors( read_matrix(b, n, 1, nameB) , nameB) )	{	free(a); free(x0); free(x); free(b); return 3;	}}	else init_matrix(b, n, 1, &fb);
	...
	//m--;
	t=clock();
	solve(a, x0, x, b, tau, m, n);
	t=clock()-t;
	printf("\nANSWER...\n");
	printf("\nVector  x:\n"); print_matrix( x, n, 1);
 	printf("\nTime: %lf\n", t/CLOCKS_PER_SEC);
	free(a); free(x0); free(x); free(b);
	return 0;
}
