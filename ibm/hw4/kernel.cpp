...
kernel::kernel (int thread_quantity, data_bus *common_data)
{
  ...
  m_kernel_data = new solver_unit [m_thread_quantity];

  for (int i = 0; i < m_thread_quantity; i++)
    m_kernel_data[i].set_arg (m_thread_quantity, i, common_data->m_nx, common_data->m_ny, common_data->m_eps, common_data);
